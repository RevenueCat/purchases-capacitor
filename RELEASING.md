...
1. Do `npm dist-tag add @revenuecat/purchases-capacitor@x.x.x latest`. Being x.x.x the latest version of purchases-capacitor. When doing the publish to npm, the latest version is overriden with the hotfix, so we need to set it back to the latest version.
